...
- [x] 5. EventBus
  - [x] 5.1 先写 EventBus 单元测试
    ...
    - _设计文档：EventBus 章节_
  - [x] 5.2 实现 `EventBus` 类（`engine/event_bus.py`）
...
- [x] 6. BuffManager
  - [x] 6.1 先写 BuffManager 单元测试
    ...
    - _需求：4.2、4.3_
  - [x] 6.2 实现 `BuffManager` 类（`engine/buff_manager.py`）
...
- [x] 7. EffectResolver
  ...
  - [x] 7.2 实现 `EffectResolver` 类（`engine/effect_resolver.py`）
  ...
  - [x]* 7.3 为 Block 先于 HP 被消耗编写属性测试
    - **属性 3：Block 先于 HP 被消耗**
    - **验证：需求 3.6**
    - 文件：`tests/property/test_damage_properties.py`
  - [x]* 7.4 为伤害结算后 HP 不超过上限编写属性测试
    - **属性 4：伤害结算后 HP 不超过上限**
    - **验证：需求 3.6、4.x**
    - 文件：`tests/property/test_damage_properties.py`
  - [x]* 7.5 为脆弱状态伤害计算编写属性测试
    - **属性 5：脆弱状态伤害计算**
    - **验证：需求 4.6**
    - 文件：`tests/property/test_damage_properties.py`
  - [x]* 7.6 为虚弱状态伤害计算编写属性测试
    - **属性 6：虚弱状态伤害计算**
    - **验证：需求 4.7**
    - 文件：`tests/property/test_damage_properties.py`
...
- [x] 8. CardPile 牌堆管理
  - [x] 8.1 先写 CardPile 单元测试
    ...
    - _需求：2（抓牌流程子流程）_
  - [x] 8.2 实现 `CardPile` 相关逻辑（`combat/card_pile.py`）
...
- [x] 9. CombatManager 战斗主循环
  - [x] 9.1 先写 CombatManager 初始化单元测试
    ...
    - _需求：1.1、1.2、1.3_
  - [x] 9.2 实现 `CombatManager` 初始化逻辑（`combat/manager.py`）
  ...
  - [x] 9.4 实现 `play_card`、`use_potion`、`end_turn` 方法
  ...
  - [x] 9.6 实现敌人行动阶段逻辑
    ...
    - _需求：5.2、5.4、5.5、5.6_
  - [x] 9.7 实现 `get_state` 和 `get_legal_actions` 方法
  ...
  - [x] 9.8 先写合法动作和 preview_damage 单元测试
  ...
  - [x]* 9.9 为初始化后 State 完整性编写属性测试
    - **属性 1：初始化后 State 完整性**
    - **验证：需求 1.1、1.2**
    - 文件：`tests/property/test_combat_properties.py`
  - [x]* 9.10 为非法资源 ID 拒绝初始化编写属性测试
    - **属性 2：非法资源 ID 拒绝初始化**
    - **验证：需求 1.3**
    - 文件：`tests/property/test_combat_properties.py`
  - [x]* 9.11 为 exhaust 牌进入 exhaust_pile 编写属性测试
    - **属性 9：exhaust 牌进入 exhaust_pile**
    - **验证：需求 3.2（隐含）**
    - 文件：`tests/property/test_combat_properties.py`
  - [x]* 9.12 为不可打出的牌不出现在合法动作中编写属性测试
    - **属性 10：不可打出的牌不出现在合法动作中**
    - **验证：需求 8.1、8.3**
    - 文件：`tests/property/test_combat_properties.py`
  - [x]* 9.13 为合法动作可执行性保证编写属性测试
    - **属性 11：合法动作可执行性保证**
    - **验证：需求 8.4**
    - 文件：`tests/property/test_combat_properties.py`
  - [x]* 9.14 为所有敌人死亡时战斗胜利编写属性测试
    - **属性 14：所有敌人死亡时战斗胜利**
    - **验证：需求 2（死亡检查）、7.8**
    - 文件：`tests/property/test_combat_properties.py`
  - [x]* 9.15 为 Player HP≤0 时战斗失败编写属性测试
    - **属性 15：Player HP≤0 时战斗失败**
    - **验证：需求 2（死亡检查）、7.8**
    - 文件：`tests/property/test_combat_properties.py`
  - [x]* 9.16 为 sequential_loop 敌人循环顺序编写属性测试
    - **属性 16：sequential_loop 敌人按固定顺序循环**
    - **验证：需求 5.2**
    - 文件：`tests/property/test_combat_properties.py`
  - [x]* 9.17 为 preview_damage 与实际伤害一致编写属性测试
    - **属性 17：preview_damage 与实际伤害一致**
    - **验证：设计决策（伤害预览计算）**
    - 文件：`tests/property/test_combat_properties.py`
  - [x]* 9.18 为 Intent 列表与 Move intents 字段一致编写属性测试
    - **属性 19：Intent 列表与 Move 的 intents 字段一致**
    - **验证：需求 5.5**
    - 文件：`tests/property/test_combat_properties.py`
  - [x]* 9.19 为药水使用后从槽位移除编写属性测试
    - **属性 20：药水使用后从槽位移除**
    - **验证：需求 9.6**
    - 文件：`tests/property/test_combat_properties.py`
...
- [x] 11. Runner（SingleBattleRunner + CampaignRunner）
  - [x] 11.1 先写 Runner 单元测试
...
- [x] 12. Bridge（ZmqBridge）
  ...
  - [x] 12.2 实现 `ZmqBridge` 类（`bridge/zmq_bridge.py`）
